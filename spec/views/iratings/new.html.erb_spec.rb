...
describe "/iratings/new.html.erb" do
  include IratingsHelper

  before(:each) do
    assigns[:irating] = stub_model(Irating,
      :new_record? => true,
      :easiness => 1,
      :interest => 1,
      :work_load => 1,
  ...
  it "renders new irating form" do
    render

    response.should have_tag("form[action=?][method=post]", iratings_path) do
